...
template <class Container_,
          class Comparison_ = std::less<typename Container_::value_type> >
 ...
 * Class: SizeStorage
 *
 * This class expresses how size should be stored for various containers to be
 * able
 * to deduce it in constant time. By default it is stored explicitly, so that if
 * the
 * Container is std::list everything works. However, specialization is available
 * for
...
struct CountingBackInserter : public std::back_insert_iterator<C>,
  ...
  CountingBackInserter(C& c) : ParentIterator(c) {}
  ...
  Self operator++(int i) {
    Self tmp = *this;
    ParentSize::operator++(i);
    ParentIterator::operator++(i);
    return tmp;
  }
};
 ...
 *
 * Performs the same task as std::back_insert_iterator<Container>, but as a
 * functor.
 */
template <class Container_>
class PushBackFunctor {
 public:
  ...
  PushBackFunctor(Container& container) : container_(container) {}
...
PushBackFunctor<Container> make_push_back_functor(Container& container) {
  return PushBackFunctor<Container>(container);
}
...
class InsertFunctor {
 public:
  ...
  InsertFunctor(Container& container) : container_(container) {}
...
template <class Container>
InsertFunctor<Container> make_insert_functor(Container& container) {
...
struct ContainerTraits<vector<T>, Comparison_> {
  ...
  static void push_front(Container& c, const_reference x) {
    c.insert(c.begin(), x);
  }
};
