...
class AssociativeMap
    : public boost::associative_property_map<UniquePairAssociativeContainer_> {
 public:
  typedef boost::associative_property_map<UniquePairAssociativeContainer_>
      Parent;

 public:
  // FIXME: takes begin, end, and initializes with the reverse
  AssociativeMap() : container(), Parent(container) {}
...
class BinarySearchMap {
 ...
 public:
  BinarySearchMap(Index bg, Index end, Comparison cmp = Comparison())
...
struct OffsetMap {
  ...
  OffsetMap(From bg_from, To bg_to) : bg_from_(bg_from), bg_to_(bg_to) {}
  ...
  template <class NewFrom_>
  struct rebind_from {
    typedef OffsetMap<NewFrom_, To_> other;
  };
  template <class NewTo_>
  struct rebind_to {
...
template <class From_, class To_>
OffsetMap<From_, To_> make_offset_map(From_ bg_from, To_ bg_to) {
...
struct OffsetBeginMap {
  ...
  OffsetBeginMap(const From& from, const To& to) : from_(from), to_(to) {}

  value_type operator[](key_type i) const {
    return to_.begin() + (i - from_.begin());
  }
...
class ChainMap {
 public:
  ...
  ChainMap(const Map1& m1, const Map2& m2) : m1_(m1), m2_(m2) {}
...
class ThroughMap {
 public:
  ...
  ThroughMap(const Map& map, const Functor& functor)
      : map_(map), functor_(functor) {}

  result_type operator()(first_argument_type a) const {
    return functor_(map_[a]);
  }
...
ThroughMap<Functor, Map> evaluate_through_map(const Map& map,
