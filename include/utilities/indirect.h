...
struct IndirectComparison {
  ...
  IndirectComparison(const Comparison& cmp) : cmp_(cmp) {}
...
class FirstComparison {
 public:
  ...
  FirstComparison(Comparison cmp) : cmp_(cmp) {}
...
struct ThreeOutcomeCompare : public Comparison {
  ...
  int compare(first_argument_type a, second_argument_type b) const {
    if (this->operator()(a, b))
      return -1;
    else if (this->operator()(b, a))
      return 1;
    else
      return 0;
  }
...
class ThroughEvaluatorComparison {
 public:
  ...
  ThroughEvaluatorComparison(const Evaluator& eval) : eval_(eval) {}

  template <class T>
  bool operator()(T a, T b) const {
    return (eval_(a) < eval_(b));
  }
...
// Iterates over the difference of the two sorted sequences, dereferencing into
// the first sequence
...
class difference_iterator
 ...
 public:
  ...
  difference_iterator(Iterator1 cur1, Iterator1 end1, Iterator2 cur2,
                      Iterator2 end2,
                      const StrictWeakOrdering& cmp = StrictWeakOrdering())
      : cur1_(cur1), end1_(end1), cur2_(cur2), end2_(end2), cmp_(cmp) {
    catchup();
  }
 ...
 private:
  ...
  bool equal(const difference_iterator& other) const {
...
template <class Iterator1, class Iterator2, class StrictWeakOrdering>
difference_iterator<Iterator1, Iterator2, StrictWeakOrdering>
make_difference_iterator(Iterator1 cur1, Iterator1 end1, Iterator2 cur2,
                         Iterator2 end2, const StrictWeakOrdering& cmp) {
  return difference_iterator<Iterator1, Iterator2, StrictWeakOrdering>(
      cur1, end1, cur2, end2, cmp);
}

// Iterates over the intersection of the two sorted sequences, dereferencing
...
class intersection_iterator
 ...
 public:
  ...
  intersection_iterator(Iterator1 cur1, Iterator1 end1, Iterator2 cur2,
                        Iterator2 end2,
                        const StrictWeakOrdering& cmp = StrictWeakOrdering())
      : cur1_(cur1), end1_(end1), cur2_(cur2), end2_(end2), cmp_(cmp) {
    catchup();
 ...
 private:
  ...
  bool equal(const intersection_iterator& other) const {
    return (cur1_ == other.cur1_) && (cur2_ == other.cur2_);
  }
  reference dereference() const { return *cur1_; }
...
template <class Iterator1, class Iterator2, class StrictWeakOrdering>
intersection_iterator<Iterator1, Iterator2, StrictWeakOrdering>
make_intersection_iterator(Iterator1 cur1, Iterator1 end1, Iterator2 cur2,
                           Iterator2 end2, const StrictWeakOrdering& cmp) {
  return intersection_iterator<Iterator1, Iterator2, StrictWeakOrdering>(
      cur1, end1, cur2, end2, cmp);
}
