...
struct OrderContainer {
  ...
  typedef boost::multi_index_container<
      Element,
      bmi::indexed_by<bmi::random_access<bmi::tag<order> > /* order index */
                      > > Container;
...
struct ElementComparison
    : public std::binary_function<const typename Container_::value_type*,
  ...
  bool operator()(const Element* a, const Element* b) const {
    return cmp_(container_.iterator_to(*a), container_.iterator_to(*b));
  }
...
struct OrderConsistencyContainer {
  ...
  typedef typename Container::template index<order>::type OrderedContainer;
  typedef
      typename Container::template index<consistency>::type ConsistentContainer;
