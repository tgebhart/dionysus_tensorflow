...
template <class Simplex_,
          class SimplexOrderIndex_ = bmi::ordered_unique<
              bmi::identity<Simplex_>, typename Simplex_::VertexComparison> >
class Filtration {
 private:
  struct order {};  // tag
 ...
 public:
  ...
  typedef b::multi_index_container<
      Simplex, bmi::indexed_by<SimplexOrderIndex,
  ...
  Index find(const Simplex& s) const {
    return bmi::project<order>(container_, container_.find(s));
  }

  // Modifiers
  template <class Comparison>
  void sort(const Comparison& cmp = Comparison()) {
    container_.template get<order>().sort(cmp);
  }
  void push_back(const Simplex& s) {
    container_.template get<order>().push_back(s);
  }
  void transpose(Index i) {
    container_.template get<order>().relocate(i, i + 1);
  }
  void clear() { container_.template get<order>().clear(); }
  template <class Iter>
  void rearrange(Iter i) {
    container_.template get<order>().rearrange(i);
  }
  ...
  std::ostream& operator<<(std::ostream& out) const {
    std::copy(begin(), end(), std::ostream_iterator<Simplex>(out, "\n"));
    return out;
  }
...
std::ostream& operator<<(std::ostream& out, const Filtration<S, SOI>& f) {
...
class ThroughFiltration {
 public:
  ...
  ThroughFiltration(const Filtration& filtration, const Functor& functor)
      : filtration_(filtration), functor_(functor) {}

  result_type operator()(first_argument_type a) const {
    return functor_(filtration_.simplex(a));
  }
...
ThroughFiltration<Functor, Filtration> evaluate_through_filtration(
    const Filtration& filtration, const Functor& functor) {
...
class DimensionFunctor {
 public:
  ...
  Dimension operator()(key_type i) const {
    return filtration_.simplex(map_[i]).dimension();
  }
...
template <class Map, class Filtration>
DimensionFunctor<Map, Filtration> make_dimension_functor(
    const Map& map, const Filtration& filtration) {
