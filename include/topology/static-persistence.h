...
struct PairCycleData : public Data_ {
  ...
  PairCycleData(Index p = Index(), const Cycle& z = Cycle(),
                const Data& d = Data())
...
class StaticPersistence {
 public:
  ...
  typedef typename ContainerTraits_::template rebind<Element>::other
      ContainerTraits;
  ...
  typedef typename ChainTraits_::template rebind<OrderIndex>::other ChainTraits;
  typedef typename ChainTraits::Chain Chain;
  typedef Chain Cycle;
  ...
  StaticPersistence(const Filtration& f)
      : ocmp_(order_) {
    initialize(f);
  }
  ...
  class SimplexMap;
  template <class Filtration>
  SimplexMap<Filtration> make_simplex_map(const Filtration& filtration) const {
    return SimplexMap<Filtration>(*this, filtration);
  }
  ...
  class OrderModifier {
   public:
    ...
    void operator()(iterator i, const Functor& f) {
      order_.modify(i, f);
    }
  ...
  void pair_simplices(iterator bg, iterator end, bool store_negative = false,
 ...
 protected:
  ...
  void set_pair(iterator i, OrderIndex j) {
    order_.modify(i, boost::bind(&OrderElement::set_pair, bl::_1, j));
  }  // i->set_pair(j)
  ...
  void swap_cycle(iterator i, Cycle& z) {
    order_.modify(
        i, boost::bind(&OrderElement::swap_cycle, bl::_1, boost::ref(z)));
  }  // i->swap_cycle(z)
