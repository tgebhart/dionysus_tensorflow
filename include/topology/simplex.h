 ...
 * a `boost::compressed_pair` (so if data is an Empty class which is the
 * default,
 * it incurs no space overhead). The class knows how to compute its own
 * <boundary()>.
 *
 * Parameter:
 *   V -            vertex type
...
class Simplex {
 public:
  ...
  /// Copy constructor
  Simplex(const Self& other) : vdpair_(other.vertices(), other.data()) {}
  ...
  Simplex(Iterator bg, Iterator end, const Data& d = Data()) {
    join(bg, end);
    data() = d;
  }
  /// Constructor: Simplex(VertexContainer v)
  /// Initialize simplex by copying the given VertexContainer
  Simplex(const VertexContainer& v, const Data& d = Data()) : vdpair_(v, d) {
  ...
  /* Classes: Comparisons
   *
   * VertexComparison -           compare simplices based on the lexicographic
   * ordering of their <vertices()>
   * VertexDimensionComparison -  compare simplices based on the lexicographic
   * ordering of their <vertices()> within each dimension
   * DataComparison -             compare simplices based on their <data()>
   * DataDimensionComparison -    compare simplices based on their <data()>
...
struct Simplex<V, T>::DataEvaluator {
  ...
  result_type operator()(const first_argument_type& s) const {
    return s.data();
  }
};
...
// TODO: class DirectSimplex - class which stores indices of the simplices in
// its boundary
// TODO: class CompactSimplex<V, T, N> - uses arrays instead of vectors to store
// simplices
