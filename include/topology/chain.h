...
/**
 * Class: ChainWrapper
 * Class storing a chain of simplices. Stores items in the order defined by
 * ConsistencyCmp.
 * The actual order of the elements is defined by OrderCmp. Instances of those
 * classes are not stored in Chain for efficiency, and are passed as arguments
 * to those methods
 * that require them.
 *
 * \ingroup topology
...
class ChainWrapper : public Container_, public SizeStorage<Container_> {
 public:
  ...
  typedef
      typename boost::iterator_value<typename Container::iterator>::type Item;
  /// @}
 ...
 public:
  ...
  /// @{
  /** Add c to *this assuming both Chains are sorted in increasing order
   * according to cmp. */
  template <class ConsistencyComparison>
  Self& add(const Self& c, const ConsistencyComparison& cmp);

  void swap(ChainWrapper& c);  ///< Swaps the contents of c and *this (like
                               ///STL's swap destroys c)
  void clear();

  template <class ConsistencyComparison>
  void sort(const ConsistencyComparison&
                cmp);  ///< Sort elements to enforce consistency
  ...
  const_reference top(
      const OrderComparison& cmp) const;  ///< First element in cmp order

  boost::optional<const_iterator> contains(
      const_reference x) const;  ///< tests whether chain contains x
  boost::optional<iterator> contains(
      const_reference x);  ///< tests whether chain contains x
  /// @}

  /// \name Debugging
  /// @{
  template <class OrderComparison>
  const_reference get_first(
      const OrderComparison& cmp) const;  ///< First element in cmp order
  ...
  /// Technically, it's an internal operation, and should be followed by a
  /// sort()
