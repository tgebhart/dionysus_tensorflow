 ...
 * Class: PDPoint
 *
 * Stores birth-death pair plus any additional information provided by `Data`
 * template parameter.
 */
...
class PDPoint {
 public:
  ...
  PDPoint(const PDPoint& other) : point_(other.point_) {}
...
boost::optional<Point> make_point(Iterator i, const Evaluator& evaluator,
 ...
 *
 * Stores birth-death pairs, i.e. points in the extended plane. Each point can
 * also store
...
class PersistenceDiagram {
 public:
  ...
  PersistenceDiagram(Dimension dimension) : dimension_(dimension) {}
...
std::ostream& operator<<(std::ostream& out,
                         const PersistenceDiagram<Data>& pd) {
  return (pd.operator<<(out));
}

// Function: init_diagram_vector()
template <class Diagrams, class Iterator, class Evaluator,
          class DimensionExtractor>
void init_diagrams(Diagrams& diagrams, Iterator bg, Iterator end,
                   const Evaluator& evaluator = Evaluator(),
                   const DimensionExtractor& dimension = DimensionExtractor());

template <class Diagrams, class Iterator, class Evaluator,
          class DimensionExtractor, class Visitor>
void init_diagrams(Diagrams& diagrams, Iterator bg, Iterator end,
                   const Evaluator& evaluator = Evaluator(),
                   const DimensionExtractor& dimension = DimensionExtractor(),
                   const Visitor& visitor = Visitor());
...
struct Linfty {
  RealType operator()(const Point1& p1, const Point2& p2) const {
    return std::max(std::abs(p1.x() - p2.x()), std::abs(p1.y() - p2.y()));
  }

  template <class Point>
  RealType diagonal(const Point& p) const {
    return std::abs(p.y() - p.x()) / 2;
  }
};
...
RealType bottleneck_distance(const Diagram1& dgm1, const Diagram2& dgm2) {
  return bottleneck_distance(
      dgm1, dgm2, Linfty<typename Diagram1::Point, typename Diagram2::Point>());
}

template <class Diagram>
RealType wasserstein_distance(const Diagram& dgm1, const Diagram& dgm2,
                              unsigned p);
